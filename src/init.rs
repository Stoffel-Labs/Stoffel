...
fn create_python_project(path: &Path, config: &StoffelConfig) -> Result<(), String> {
    ...
    let pyproject_content = format!(r#"[build-system]
...
"#,
    ...
    let main_py_content = format!(r#"#!/usr/bin/env python3
"""
{} - {}
...
async def main():
    ...
    program = StoffelProgram("secure_computation.stfl")
    program.compile()
    ...
    print("   ✓ Connected to {} MPC parties")
...
"#,
    ...
    let stfl_content = format!(r#"# {} - Secure computation program
# Generated by Stoffel CLI
# Protocol: {}, Parties: {}, Field: {}
#
...
"#,
    ...
    fs::write(path.join("secure_computation.stfl"), stfl_content)
        .map_err(|e| format!("Failed to write secure_computation.stfl: {}", e))?;

    // Create test file
    let test_content = r#"#!/usr/bin/env python3
"""
