...
///
/// Creates n zero-threshold engines in-process, shares a known value (42), then
/// calls `open_share_in_exp` on each engine. Verifies the result equals
/// `G1::generator() * Fr::from(42)`.
///
/// HoneyBadger exponent opening deliberately fails closed for Byzantine-threshold
/// sessions because `RobustShare` has no authenticated polynomial commitment.
/// AVSS integration tests cover the verifiable threshold path.
...
///
/// Simulates what `random_share` would produce in a zero-threshold HoneyBadger
/// session: all parties hold shares of the same random secret (generated by
/// `compute_shares`). Then each party calls `open_share_in_exp` with
/// G1::generator(). All parties should get identical, non-trivial public key bytes.
///
