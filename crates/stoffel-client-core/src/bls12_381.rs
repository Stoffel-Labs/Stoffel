...
//! `stoffelcrypto::RobustShare<ark_bls12_381::Fr>`; it does not import the native
//! MPC stack.
...
impl fmt::Display for CryptoError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::InvalidCanonicalField => f.write_str("invalid canonical BLS12-381 field bytes"),
            ...
            Self::InvalidPartyCount => f.write_str("party count has no BLS12-381 FFT domain"),
            Self::InconsistentShare(id) => {
                write!(f, "mask share {id} is inconsistent with the polynomial")
            }
            Self::Serialization => f.write_str("failed to serialize canonical BLS12-381 bytes"),
        }
    }
}
...
/// Encode a nonnegative private integer as a canonical compressed BLS12-381 scalar.
